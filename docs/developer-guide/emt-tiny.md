# Edge Microvisor Toolkit Tiny

Edge Microvisor Toolkit Tiny is a custom, minimal build of Edge Microvisor Toolkit.
It is intended for use in the workflows of Edge Manageability Framework and Edge Microvisor
Toolkit Standalone Node. Tiny has been introduced to replace previously used HookOS in
builds. It runs in RAM memory and installs the Edge Microvisor Toolkit operating system.

## Building the Tiny image

Edge Microvisor Toolkit Tiny is built from the same baseline as other microvisor OS images
and is generated as a set of `initramfs` and `vmlinuz` image files. The characteristics of
the resulting image are defined in [edge-image-tiny.json](https://github.com/open-edge-platform/edge-microvisor-toolkit/blob/3.0/toolkit/imageconfigs/edge-image-tiny.json) configuration file. The OS includes
...
as well as
[Tiny specific packages](https://github.com/open-edge-platform/edge-microvisor-toolkit/blob/3.0/toolkit/imageconfigs/packagelists/tiny-packages.json).
...
To build the Tiny OS image, run the following command:

```bash
sudo make image -j8 REBUILD_TOOLS=y REBUILD_PACKAGES=n CONFIG_FILE=./imageconfigs/edge-image-tiny.json
```

The build results in a compressed `emt-tiny.tar.gz` file.
...
the [generate-tiny-initramfs.sh](https://github.com/open-edge-platform/edge-microvisor-toolkit/blob/3.0/toolkit/imageconfigs/scripts/generate-tiny-initramfs.sh) bash script. See the usage example:

```bash
sudo toolkit/imageconfigs/scripts/generate-tiny-initramfs.sh \
  -f <emt-tiny.tar.gz> -o <output_images_dir>
```

* The `<emt-tiny.tar.gz>` is the output "rootfs.tar.gz" file generated by the Tiny OS build.
...
customizations in [Edge Manageability Framework](#orchestrator-build-with-tiny-new-workflow)
or [Edge Microvisor Toolkit Standalone Node](#microvisor-build-with-tiny-new-workflow) builds.

## Integration with Edge Manageability Framework and Edge Microvisor Toolkit Standalone Node

The primary components in Edge Microvisor Toolkit Tiny, that is *device-discovery*,
*tink-worker* are required for provisioning of Edge Manageability Framework (orchestrator)
and are built as RPMs (from open source) and included in an output *emt-tiny.tar.gz* image file by
...
### Orchestrator Build with Tiny (new workflow)

When using Edge Microvisor Toolkit Tiny in the build workflow, the following RPM
packages are run as native systemd services in the Edge Microvisor Toolkit OS:

* Caddy and Fluent Bit are existing RPM packages which are included in Tiny OS.
...
Tiny provides `vmlinuz` and `initramfs` images for use in installer builds of Edge
...
### Microvisor Build with Tiny (new workflow)

When using Edge Microvisor Toolkit Tiny in the build workflow, the following
...
Tiny also includes efibootmgr, gawk, lvm2, net-tools, and parted packages to support creation of
